...
#    distributed under the License is distributed on an "AS IS" BASIS, WITHOUTF
...
set +e
VLAN_RANGE=`/usr/local/src/neutron-ci/vlan_allocation.py -a $VMID`
if [ ! -z "$VLAN_RANGE" ]
then
...
nohup /usr/local/src/neutron-ci/jobs/build_devstack.sh > /home/jenkins-slave/logs/devstack-build-log-$ZUUL_UUID 2>&1 &
pid_devstack=$!

# Building and joining HyperV nodes
echo `date -u +%H:%M:%S` "Started building & joining Hyper-V node: $hyperv01"
nohup /usr/local/src/neutron-ci/jobs/build_hyperv.sh $hyperv01 > /home/jenkins-slave/logs/hyperv-build-log-$ZUUL_UUID-$hyperv01 2>&1 &
pid_hv01=$!

echo `date -u +%H:%M:%S` "Started building & joining Hyper-V node: $hyperv02"
nohup /usr/local/src/neutron-ci/jobs/build_hyperv.sh $hyperv02 > /home/jenkins-slave/logs/hyperv-build-log-$ZUUL_UUID-$hyperv02 2>&1 &
pid_hv02=$!

# Waiting for devstack threaded job to finish
wait $pid_devstack
echo "devstack build log can be found in http://64.119.130.115/neutron/$ZUUL_CHANGE/$ZUUL_PATCHSET/devstack-build-log-$ZUUL_UUID"

# Wait for both nodes to finish building and joining
wait $pid_hv01
echo "$hyperv01 build log can be found in http://64.119.130.115/neutron/$ZUUL_CHANGE/$ZUUL_PATCHSET/hyperv-build-log-$ZUUL_UUID-$hyperv01"

wait $pid_hv02
echo "$hyperv02 build log can be found in http://64.119.130.115/neutron/$ZUUL_CHANGE/$ZUUL_PATCHSET/hyperv-build-log-$ZUUL_UUID-$hyperv02"
