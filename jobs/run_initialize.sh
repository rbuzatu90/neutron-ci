...
while [ $VM_OK -ne 0 ]; do
    ...
    echo "VM connectivity NOT OK, rebooting VM"
        ...
        if [ $status -eq 0 ]; then
            ...
            echo "nova console-log $VMID:"; nova console-log "$VMID"; echo "Failed listening for ssh port on devstack"            echo "Deleting VM $VMID"
            nova delete $VMID
        fi
    fi
done
...
scp -v -r -o "StrictHostKeyChecking no" -o "UserKnownHostsFile /dev/null" -i $DEVSTACK_SSH_KEY /usr/local/src/neutron-ci-2016/devstack_vm/* ubuntu@$FLOATING_IP:/home/ubuntu/
...
nohup /usr/local/src/neutron-ci-2016/jobs/build_devstack.sh $hyperv01_ip $hyperv02_ip > /home/jenkins-slave/logs/devstack-build-log-$ZUUL_UUID.log 2>&1 &
pid_devstack=$!

# Building and joining HyperV nodes
echo `date -u +%H:%M:%S` "Started building & joining Hyper-V node: $hyperv01"
nohup /usr/local/src/neutron-ci-2016/jobs/build_hyperv.sh $hyperv01 > /home/jenkins-slave/logs/hyperv-build-log-$ZUUL_UUID-$hyperv01.log 2>&1 &
pid_hv01=$!

echo `date -u +%H:%M:%S` "Started building & joining Hyper-V node: $hyperv02"
nohup /usr/local/src/neutron-ci-2016/jobs/build_hyperv.sh $hyperv02 > /home/jenkins-slave/logs/hyperv-build-log-$ZUUL_UUID-$hyperv02.log 2>&1 &
pid_hv02=$!
